...
type stack_value =
  ...
  | Error
  | Unit
  | Closure of stack_value   (* function name *)
               * stack_value (* parameter name *)
    ...
    if not (is_letter first_char || first_char = '_') then
      ...
      let rec check_rest i =
        if i >= String.length s then true
          ...
          let ch = s.[i] in
          if is_letter ch || is_digit ch || ch = '_' then
            check_rest (i + 1)
          else
            false
      in
...
let write_lines (filename : string) (stack : stack) : unit =
  ...
  try
    List.iter
      (fun line ->
         output_string oc (string_of_stack_value line ^ "\n"))
      stack;
    close_out oc
  with e ->
    close_out_noerr oc;
...
let arithmetic (op : operation) (stk : stack) (env : env_stack)
  : stack * env_stack =
  match op with
  | Add ->
      (match stk with
...
let arithmetic_helper (op : operation) (stk : stack) (env : env_stack)
  : stack * env_stack =
  match stk with
...
let println (out : out_channel) (stk : stack) (env : env_stack)
  : stack * env_stack =
  match stk with
  | [] -> pushError stk env
  | v :: rest ->
...
let rec fetch_from_environment_list (name : string) (env_list : environment)
  : stack_value =
  match env_list with
  | [] -> Error
  | (n, v) :: rest ->
...
let add_to_environment_list (name : stack_value) (value : stack_value)
    (env_list : environment) : environment =
  (name, value) :: env_list

let remove_from_environment_list (name : stack_value)
    (env_list : environment) : environment =
  let key = string_of_stack_value name in
  List.filter (fun (n, _) -> string_of_stack_value n <> key) env_list

let replace_in_environment_list (name : stack_value) (value : stack_value)
    (env_list : environment) : environment =
  let env_without_name = remove_from_environment_list name env_list in
  add_to_environment_list name value env_without_name

let rec resolve_names (stk : stack) (env : env_stack) : stack =
  List.map
    (function
      | Name n ->
...
let boolean_logic (op : boolean_op) (stk : stack) (env : env_stack)
  : stack * env_stack =
  match op with
  | And ->
      (match stk with
...
(* Check validity of a user-defined function / parameter name *)
...
(* return: return top value from function stack *)
  ...
  | arg :: Name funName :: rest ->
      ...
      | Closure (Name fn, Name param, captured_env, body) ->
          ...
          if actualValue = Error then
            ...
            (* New environment frame for the call *)
            let call_env_frame =
            ...
            let rec run cmds fstk fenv =
              ...
              | cmd :: more ->
                  ...
                  let tokens = tokenize_command trimmed in
                  let (nstk, nenv) =
                    match tokens with
            ...
            let result_stack = run body [] [call_env_frame] in
            let ret =
...
let interpreter ((input : string), (output : string)) : unit =
  ...
  let exec_cmd ?(resolve = true) f (stk : stack) (env : env_stack)
    : stack * env_stack =
    let adjusted_stk = if resolve then resolve_names stk env else stk in
    f adjusted_stk env
  in

  let rec execute (commands : string list) (stk : stack) (env : env_stack)
    : stack =
    match commands with
    | [] -> stk
    | cmd :: rest ->
        ...
        if trimmed_cmd = "" then
          execute rest stk env
        else
          (* split into words for fun/inOutFun detection *)
          let words =
            String.split_on_char ' ' trimmed_cmd
            |> List.filter (fun s -> s <> "")
          in
          match words with
          (* -------- function definition: fun name param (or inOutFun) -------- *)
          | ("fun" | "inOutFun") :: funName :: param :: [] ->
              if not (funcNameValid funName && funcNameValid param) then
                let (stk', env') = pushError stk env in
                execute rest stk' env'
              else
                (* collect body until matching top-level funEnd *)
                let rec collect_body acc depth cmds =
                  match cmds with
                  | [] -> (List.rev acc, [])
                  | line :: more ->
                      ...
                      begin match ws with
                      | ("fun" | "inOutFun") :: _ ->
                          collect_body (line :: acc) (depth + 1) more
                      | ["funEnd"] ->
                          if depth = 0 then
                            (List.rev acc, more)
                          else
                            collect_body (line :: acc) (depth - 1) more
                      | _ ->
                          collect_body (line :: acc) depth more
                      end
                in
                let (body, remaining) = collect_body [] 0 rest in

                (* capture environment from top frame *)
                let captured_env =
                  match env with
                  | (current_env, _) :: _ -> current_env
                  | [] -> []
                in
                let closure =
                  Closure (Name funName, Name param, captured_env, body)
                in
                ...
                let new_env_stack : env_stack =
                  ...
                  | (current_env, old_stack) :: outer ->
                      let new_env =
                        if check_environment_list funName current_env then
                          replace_in_environment_list (Name funName) closure current_env
                        else
                          add_to_environment_list (Name funName) closure current_env
                      in
                      (new_env, old_stack) :: outer
                  | [] ->
                      [ (add_to_environment_list (Name funName) closure [], []) ]
                in

                (* defining a function pushes :unit: *)
                execute remaining (Unit :: stk) new_env_stack
  ...
  List.iter
    (fun line -> output_string oc (string_of_stack_value line ^ "\n"))
    final_stack;
  close_out oc
